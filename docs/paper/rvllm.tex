...
rvLLM with FP16 inference achieves 12{,}312~tok/s serving Qwen2.5-7B at $N\!=\!128$ (direct engine),
\textbf{0.85$\times$ Python vLLM~0.18 direct engine (0.67--0.88$\times$ via HTTP)}
...
(6)~FlashAttention-3 v3 with cp.async and split-KV for decode, (7)~no-fallback kernel validation, and (8)~a zero-copy
...
\begin{enumerate}
  \item A modular 23-crate Rust architecture for LLM inference, with GPU-resident
        ...
        decoding.
  \item 50~CUDA kernels including FlashAttention-3 v3 with cp.async bulk copies and split-KV,
        paged KV cache support, GPU argmax, and fused residual-RMSNorm. No-fallback
        startup validation ensures every kernel loads or the server fails fast.
  \item Verified benchmark results: 12{,}312~tok/s on H100 (FP16, Qwen2.5-7B, $N\!=\!128$, direct engine),
        achieving 0.67--0.88$\times$ of Python vLLM via HTTP, with coherence verified across 14{,}620 total
