...
The system comprises 23~Rust crates and 15~pre-compiled CUDA PTX kernels, compiling
to a single 16\,MB static binary. In head-to-head benchmarks on a single A100~80\,GB
SXM4, rvLLM with FP16 inference achieves 8{,}339~tok/s serving Qwen2.5-1.5B,
\textbf{matching Python vLLM~0.18 throughput at production batch sizes}
($N\!=\!48$--$128$) while offering 20$\times$ faster startup, a 31$\times$ smaller
binary, and 3$\times$ less CPU memory. At higher concurrency ($N\!>\!256$), Python
vLLM scales further due to its mature CUDA graph and Triton kernel optimisations.
The throughput parity at production batch sizes derives from three reinforcing
mechanisms: (1)~GPU-resident greedy decoding via an argmax kernel that reduces
device-to-host transfer by $\sim$150{,}000$\times$ per step, (2)~FP16 hgemm with
tensor cores and f16 KV cache, and (3)~a zero-copy memory architecture with no
Python objects, no PyTorch tensor metadata, and no garbage collector. rvLLM is
